			...
			<tbody>
				<?php foreach($data as $data):?>
					<tr>
						...
						<td><?=date('d/m/Y',$data['invest_date']);?></td>
						<td><?=$data['amount'];?></td>
						<td><?=$data['purpose'];?></td>
						<td><?=$data['added_by'];?></td>
						<td><?=date('d/m/Y h:i a',$data['date']);?></td>
						<td>
							<a type="button" onclick="return confirm('Are You Sure?')" class="btn btn-default" href="<?=base_url()?>admin/editInvest/<?=$data['id']?>"><i class="far fa-edit"></i></a> | 
    ...
    $('#datatable').DataTable( {
        ...
        buttons: [
...

            {
				...
				messageTop: '<p align="center">Invest Report</p><br><p align="center"><?=date('d/m/Y h:i a')?></p>',
